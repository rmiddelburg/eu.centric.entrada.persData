...
//                            	 layoutdata: new sap.ui.layout.ResponsiveFlowLayoutData({weight : 2,}),
                 				parts:[ {path: 'Nameofaddresstype', type : new sap.ui.model.type.String()},
                						{path: 'Validitybegin', type : this.oTypeDate},
                						{path: 'Validityend', type : this.oTypeDate}     
                				       ],
                				formatter : function(Nameofaddresstype, Validitybegin, Validityend) {
                					return Nameofaddresstype + " (" + oBundle.getText("BEGDA") +Validitybegin + " " + oBundle.getText("ENDDA") + Validityend + ")";
                				}
                    			},
                             }),
                             formElements: [            
                                     new sap.ui.layout.form.FormElement({label: oBundle.getText("STREET"),fields: [new sap.m.Text({text: "{Street}"}),]}),
                                     new sap.ui.layout.form.FormElement({label: oBundle.getText("POSTELCODE_CITY"), fields: [new sap.m.Text({text: "{Postalcodecity}",layoutData: new sap.ui.layout.GridData({span: "L1 M2 S12",})}),
                                                                                                                             new sap.m.Text({text: "{City}"})  ]}),
                                     new sap.ui.layout.form.FormElement({label: oBundle.getText("COUNTRY"),fields: [new sap.m.Text({text: "{Nameofcountry}"}),]}),
                                     new sap.ui.layout.form.FormElement({
                                         label: "",
                                         fields: [new sap.m.Button({icon : "sap-icon://edit",layoutData: new sap.ui.layout.GridData({span: "L1 M1 S2", indent: "L10 M10 S8",}),press: oController.onAddressEdit}), 
...
//			visible : !jQuery.device.is.desktop,
		...
		this.addressItemTemplate = new sap.m.CustomListItem({content : [
		                                                                this.addressForm,
		                                                                ]});
